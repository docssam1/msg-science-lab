...
import {createEverydayAssembly} from './everyday-objects-3d.js';
...
export function mountLab(host, {mode='compare',onChange=()=>{},onPick=()=>{},onRelease=()=>{},onAdjust=()=>{}}={}) {
 ...
 if(measure){
  ...
  for(let i=0;i<=Math.round(scaleMax/scaleStep);i++){let value=Number((i*scaleStep).toFixed(1)),y=y0-value*pitch,major=inquiry?i%10===0:i%5===0;line([.32,y,.04],[major?.71:.55,y,.04]);if(major)tickLabels.push(label(String(value),.86,y,.06,{size:.13,width:128}));}
  tickLabels.push(label('N',.51,1.57,.05,{size:.23,width:128}));tickLabels.push(label('MAX '+scaleMax+' N',.25,-1.08,.15,{size:.13,width:390,color:'#b55844'}));
 }
