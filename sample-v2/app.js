...
const coach=createCoach({figure:$('.character-figure'),caption:$('#coach-copy'),label:$('#voice-kind'),onState:s=>{reading=s.playing;$('#start').textContent=s.playing?'Ⅱ 해설 멈추기':started?'▶ 해설 다시 듣기':'▶ 수업 시작';},onError:e=>console.warn(e.message)});
await coach.ready;coach.setEnabled(sound);
let files=coach.library.clips||{};
let toastTimer;
...
function updateMeta(){drawInk();const p=pages[current];$('#current-section').textContent=`${p.lesson}차시 · ${p.kicker.split('·')[1]||p.kicker}`;$('#page-source').textContent=`본책 ${p.source.join('·')}쪽 · 재편집 ${current+1}/${pages.length}`;$('#page-select').value=current;$('#prev').disabled=current===0;$('#next').disabled=current>=pages.length-1;$$('button[data-lesson]').forEach(b=>b.classList.toggle('selected',+b.dataset.lesson===p.lesson));$('#teacher-guide').textContent=p.teacher;$('#teacher-guide').hidden=!(isTeacher&&keysVisible);coach.preview(files[p.id]?.text||narration[p.id].text,p.id==='l1-future'?'surprise':p.assessment||p.added?'think':'listen');$('#lesson-state').textContent=p.assessment?'생각하고 답할 때는 자동으로 넘기지 않습니다.':'해설 → 활동 → 관찰 → 교재로 돌아오기';save('page',current);requestAnimationFrame(fitPageBodies);}
...
function stopVoice(){voiceToken++;coach.stop();reading=false;}
async function say(text,clipId=null){coach.setEnabled(sound);return coach.say(text,{id:clipId});}
...
function readFull(){const p=pages[current];started=true;say(p.title.replace('\n',' '),'full:'+p.id);}
async function openActivity(kind,auto=false,custom=null){inkOff();activeKind=kind;drawInk();stopVoice();activityToken++;const token=activityToken;activity?.destroy();activity=null;window.__lab=null;const wasActive=$('#workspace').classList.contains('active');$('#workspace').classList.add('active');$('#activity').setAttribute('aria-hidden','false');$('#activity-title').textContent=activityNames[kind]||'개념 다시 확인';$('#activity-kicker').textContent=['watch','balance','spring-film'].includes(kind)?'ACTUAL FOOTAGE · 실제 기록영상':'SMART LAB · 교재 연계 활동';$('#activity-status').textContent='';$('#activity-content').innerHTML='';if(!wasActive)buildBook();
 const ctx={say,status:t=>{$('#activity-status').textContent=t;if(/오류|다시 살펴|다시 확인/.test(t))coach.mood('encourage');else if(/기록했|기록했습니다|모두 확인|맞았/.test(t))coach.mood('praise');},respond:(id,text,mood)=>coach.respond(id,text,mood),load,save,setLab:l=>window.__lab=l,auto,open:openActivity,writable};
...
function navigate(i,{animated=false}={}){if(i<0||i>=pages.length)return;inkOff();stopVoice();if($('#workspace').classList.contains('active'))closeActivity();const old=current;current=i;const sameSpread=isSpread()&&Math.floor(old/2)===Math.floor(i/2);
 ...
 $('#voice-kind').textContent='우루사쌤 · OmniVoice · 수업 시작 후 재생';
}
...
$('#sound').onclick=()=>{sound=!sound;coach.setEnabled(sound);save('sound',sound);$('#sound').setAttribute('aria-pressed',sound);$('#sound').textContent=sound?'소리 켬':'소리 끔';if(!sound)stopVoice();};
...
window.__sample={pages,questions,answers,results,navigate,open:openActivity,close:closeActivity,grade,printBuild,get current(){return current},get book(){return book},get writable(){return writable},coach,get edition(){return edition},get teacher(){return isTeacher},save,load};
