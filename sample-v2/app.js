...
let clearPageSpeech=()=>{},clearActivitySpeech=()=>{};
...
function renderMobilePage(){if(innerWidth>700&&!isStudent)return;clearPageSpeech();const host=$('#mobile-reader');host.innerHTML=paper(pages[current],current,current+1);host.scrollTop=0;restoreFields(host);if(isStudent&&pages[current].assessment)clearPageSpeech=mountAssessmentSpeech(host,questions,valueFor);}
...
function showStudentGuide(){
 ...
 const guide=studentGuideFor(pages[current],current,pages.length-1,studentVisited.has(current));
...
function expression(name){const img=$('#teacher-expression');if(img){img.src=isStudent?'../assets/teacher-neutral.png':`./art/expressions/${name}.png`;img.alt=isStudent?'전신 모습의 우루사쌤':`우루사쌤 ${name==='explain'?'설명하는':'듣는'} 표정`;}}
...
async function openActivity(kind,auto=false,custom=null){if(kind==='battle'&&!isTeacher)return;if(isStudent){studentVoiceSeq++;studentEngaged=true;if(!['reading','assessment','source','credits'].includes(kind)){studentVisited.add(current);save('guide-visited',[...studentVisited]);}}currentActivityKind=kind;document.body.classList.toggle('battle-active',kind==='battle');$('#battle').setAttribute('aria-pressed',String(kind==='battle'));stopVoice();activityToken++;const token=activityToken;activity?.destroy();clearActivitySpeech();clearActivitySpeech=()=>{};activity=null;window.__lab=null;const wasActive=$('#workspace').classList.contains('active');$('#workspace').classList.add('active');$('#activity').setAttribute('aria-hidden','false');$('#activity-title').textContent=activityNames[kind]||'개념 다시 확인';$('#activity-kicker').textContent=['watch','balance','spring-film'].includes(kind)?'ACTUAL FOOTAGE · 실제 기록영상':'SMART LAB · 교재 연계 활동';$('#activity-status').textContent='';$('#activity-content').innerHTML='';if(!wasActive)buildBook();if(isStudent)showStudentGuide();
 ...
 if(kind==='assessment'){const p=pages[current];$('#activity-content').innerHTML='<div class="large-reading">'+paper(p,current)+'</div>';restoreFields($('#activity-content'));if(isStudent&&p.assessment)clearActivitySpeech=mountAssessmentSpeech($('#activity-content'),questions,valueFor);return;}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){stopVoice();document.querySelectorAll('video').forEach(v=>v.pause());}});addEventListener('pagehide',()=>{stopVoice();clearPageSpeech();clearActivitySpeech();activity?.destroy();ro.disconnect();});
