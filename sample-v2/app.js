...
function renderMobilePage(){if(innerWidth>700&&!isStudent)return;clearPageSpeech();clearPagePhoto();const host=$('#mobile-reader');host.innerHTML=paper(pages[current],current,current+1);host.scrollTop=0;restoreFields(host);if(isStudent&&pages[current].assessment){clearPageSpeech=mountAssessmentSpeech(host,questions,valueFor);clearPagePhoto=mountAssessmentPhoto(host,pages[current].printId);}}
