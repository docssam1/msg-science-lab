...
const pageGradeGroups={...gradeGroups,p11:q1.slice(0,3),p12:q1.slice(3,6),p20:q2.slice(0,3),p21:q2.slice(3,6),p22:q2.slice(6,8)};
...
function pageVoice(p){if(!isTeacher&&p.printId==='P5')return {text:'다섯 물건을 하나씩 재고 처음 예상과 비교해 보세요.',id:null};if(!isTeacher&&p.printId==='P13')return {text:'먼저 모양의 변화를 관찰하고, 힘을 없애면 어떻게 될지 예상해 보세요.',id:null};if(!isTeacher&&p.printId==='P17')return {text:'표를 보고 그래프에 점을 직접 찍어 보세요.',id:null};if(p.printId==='P18')return {text:'재질·철사 굵기·코일 지름을 한 번에 하나씩 바꿔 비교해 보세요.',id:null};return {text:files[p.id]?.text||narration[p.id]?.text||'그림을 보고 직접 관찰해 보세요.',id:p.id};}
...
function credits(){ $('#activity-content').innerHTML=`<div class="credits"><h3>교재 원본</h3><p>사용자 제공 《초과심_물리》 원본 앞표지와 본책 10–21쪽을 사용했습니다. 원본의 역사·미래 서술과 편집 보완 설명은 구분합니다.</p><h3>사진 출처</h3><p>P3–P5의 다섯 생활 물건, P7의 저울 세 종류, P19의 생활 도구 다섯 사진은 원본과 촬영자·라이선스를 <a href="./photos/credits.html" target="_blank" rel="noopener">사진 출처 목록</a>에서 확인할 수 있습니다. 사진 속 물건의 실제 무게와 앱의 가상 측정값은 다릅니다. P19 완력기는 개념 도해입니다.</p><h3>기존 영상 재사용</h3><p><a href="https://commons.wikimedia.org/wiki/File:HowaWatchWork1949.ogv" target="_blank" rel="noopener">How a Watch Works (1949)</a>의 기존 발췌 영상 두 장면과 <a href="https://commons.wikimedia.org/wiki/File:Hookeslawexample.ogv" target="_blank" rel="noopener">Hookeslawexample</a>의 기존 관찰 영상을 재사용합니다. 후자는 10·20·30 g의 수치를 증명하는 영상이 아닙니다.</p><h3>구현 자산</h3><p>StPageFlip과 Three.js의 라이선스 문서를 유지했습니다. 설명 음성은 기존 OmniVoice 클립이며, 재생 실패 시 자막으로 확인합니다. 승인된 우루사쌤 표정 이미지만 표시합니다.</p><h3>기록</h3><p>답안과 활동 기록은 이 브라우저의 저장 공간에 남습니다. 원본 문항·그림과 해설의 최종 대조는 별도 검수가 필요합니다.</p></div>`;}
