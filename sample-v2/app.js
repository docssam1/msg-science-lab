...
function closeActivity(){document.body.classList.remove('battle-active');$('#battle').setAttribute('aria-pressed','false');stopVoice();activityToken++;activity?.destroy();activity=null;window.__lab=null;$('#activity-content').innerHTML='';$('#activity').setAttribute('aria-hidden','true');$('#workspace').classList.remove('active');buildBook();if(isTeacher)$('#teach-overlay').hidden=teachStage>0;}
function navigate(i,{animated=false}={}){if(i<0||i>=pages.length)return;stopVoice();if($('#workspace').classList.contains('active'))closeActivity();const old=current;current=i;const sameSpread=isSpread()&&Math.floor(old/2)===Math.floor(i/2);
...
function credits(){ $('#activity-content').innerHTML=`<div class="credits"><h3>교재 원본</h3><p>사용자 제공 《초과심_물리》 원본 앞표지와 본책 10–21쪽을 사용했습니다. 원본의 역사·미래 서술과 편집 보완 설명은 구분합니다.</p><h3>사진 출처</h3><p>P3–P5의 생활 물건, P7의 저울, P18의 생활 도구, P9의 실제 시계 태엽 사진은 원본과 촬영자·라이선스를 <a href="./photos/credits.html" target="_blank" rel="noopener">사진 출처 목록</a>에서 확인할 수 있습니다. 사진 속 물건의 실제 무게와 앱의 가상 측정값은 다릅니다. P18 완력기는 개념 도해입니다. P10의 극소 코일·4D 프린팅·2D 재료 이미지는 가상 실사로 만든 설명용 시각화이며 실제 연구 사진이 아닙니다.</p><h3>기존 영상 재사용</h3><p><a href="https://commons.wikimedia.org/wiki/File:HowaWatchWork1949.ogv" target="_blank" rel="noopener">How a Watch Works (1949)</a>의 기존 발췌 영상 두 장면과 <a href="https://commons.wikimedia.org/wiki/File:Hookeslawexample.ogv" target="_blank" rel="noopener">Hookeslawexample</a>의 기존 관찰 영상을 재사용합니다. 후자는 10·20·30 g의 수치를 증명하는 영상이 아닙니다.</p><h3>구현 자산</h3><p>StPageFlip과 Three.js의 라이선스 문서를 유지했습니다. 설명 음성은 기존 OmniVoice 클립이며, 재생 실패 시 자막으로 확인합니다. 승인된 우루사쌤 표정 이미지만 표시합니다.</p><h3>기록</h3><p>답안과 활동 기록은 이 브라우저의 저장 공간에 남습니다. 원본 문항·그림과 해설의 최종 대조는 별도 검수가 필요합니다.</p></div>`;}
...
document.body.dataset.edition=edition;$('#edition').value=edition;document.title=editionName(edition)+' | MSG 초·과·심';$('#teacher').textContent='교안·정답 공개';$('#teacher').hidden=!isTeacher;$('#teacher-notes').hidden=!isTeacher;$('#battle').hidden=!isTeacher;
