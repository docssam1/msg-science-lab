...
let liveKind=null,liveDismissed=-1;
...
function fit(){const space=$('#book-space'),wrap=$('#book-transform');if(!space||!wrap)return;const ws=$('#workspace');if(ws.classList.contains('split')){const cs=getComputedStyle(ws),h=ws.clientHeight-parseFloat(cs.paddingTop)-parseFloat(cs.paddingBottom);ws.style.setProperty('--page-w',Math.round(Math.max(260,Math.min(ws.clientWidth*.5,(h-10)*794/1123+10)))+'px');}const w=isSpread()?1588:794,h=1123;const scale=Math.max(.05,Math.min((space.clientWidth-16)/w,(space.clientHeight-14)/h));wrap.style.width=w+'px';wrap.style.height=h+'px';wrap.style.transform=`translate(-50%,-50%) scale(${scale})`;$('#fit-info').textContent=isStudent&&!document.body.classList.contains('page-fit')?'책 안에서 스크롤·본문 확대':`전체 지면 ${Math.round(scale*100)}% · 본문 크게로 확대`;}
function toggleStudentZoom(){studentZoom=!studentZoom;save('student-zoom',studentZoom);document.body.classList.toggle('student-zoom',studentZoom);$('#focus-reading').textContent=studentZoom?'원래 크기':'본문 크게';$('#read-text').textContent=studentZoom?'원래 크기':'본문 크게';syncPageFit();}
...
const CLIPS={watch:'watch-mainspring',balance:'watch-balance','spring-film':'spring-observation'};
function inlineVideos(root){if(splitCapable())return;root.querySelectorAll('.story-video[data-action]').forEach(el=>{const clip=CLIPS[el.dataset.action];if(!clip)return;const img=el.querySelector('img'),label=el.querySelector('strong')?.textContent||activityNames[el.dataset.action];const fig=document.createElement('figure');fig.className='inline-film';fig.innerHTML=`<video controls muted playsinline preload="none" poster="${esc(img?.getAttribute('src')||'')}" aria-label="${esc(label)}"><source src="./media/${clip}.webm" type="video/webm"><source src="./media/${clip}.mp4" type="video/mp4"></video><figcaption><b>▶ 실제 영상</b> ${esc(label)}</figcaption>`;el.replaceWith(fig);});}
...
window.__sample={pages,questions,answers,results,navigate,open:openActivity,close:closeActivity,grade,report:openDailyReport,remedy:openRemedy,remedyLog,printBuild,get current(){return current},get book(){return book},get writable(){return writable},get edition(){return edition},get teacher(){return isTeacher},get live(){return liveKind},save,load};
