...
const svg=(vb,body,label)=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="${vb}" role="img" aria-label="${esc(label)}">${body}</svg>`;
const coil=(x,y,w,h,n=12)=>{
 const samples=n*24,r=w*.40,ripple=Math.min(h/n*.32,w*.12);
 let d='';for(let i=0;i<=samples;i++){const t=i/samples,a=t*n*Math.PI*2;d+=(i?'L':'M')+(x+w/2+r*Math.sin(a)).toFixed(2)+' '+(y+h*t+ripple*(Math.cos(a)-1)).toFixed(2)+' ';}return d;
};
...
export function graphSVG({points=[],max=12,step=3,labels=true,options=false}={}){
 const X=x=>90+x*14,Y=y=>360-y*24;
 return svg('0 0 650 450',`<g stroke="#c1d0d7" stroke-width="1.5" stroke-dasharray="5 5">${[10,20,30].map(x=>`<path d="M${X(x)} 72V360"/>`).join('')}${[step,step*2,step*3,step*4].map(y=>`<path d="M90 ${Y(y*12/(step*4))}H510"/>`).join('')}</g><path d="M90 56v304h459" fill="none" stroke="#173955" stroke-width="3"/><path d="M82 69l8-15 8 15M536 352l16 8-16 8" fill="none" stroke="#173955" stroke-width="3"/><text x="87" y="395" text-anchor="middle" font-size="20">0</text>${labels?[10,20,30].map(x=>`<text x="${X(x)}" y="394" text-anchor="middle" font-size="20">${x}</text>`).join('')+[step,step*2,step*3,step*4].map((y,i)=>`<text x="72" y="${360-(i+1)*72+7}" text-anchor="end" font-size="20">${y}</text>`).join(''):''}<text x="90" y="32" font-size="20" font-weight="700">늘어난 길이 (cm)</text><text x="474" y="433" font-size="20" font-weight="700">추의 무게 (g)</text>${points.length?`<path d="M90 360 ${points.map(([x,y])=>`L${X(x)} ${360-y/(step*4)*288}`).join(' ')}" fill="none" stroke="#cc6247" stroke-width="4"/>${points.map(([x,y])=>`<circle cx="${X(x)}" cy="${360-y/(step*4)*288}" r="7" fill="#cc6247"/>`).join('')}`:''}`,'추의 무게와 용수철이 늘어난 길이 그래프');}
export function graphChoices(){return svg('0 0 700 320',[0,1,2,3].map((v,i)=>{const x=32+i%2*350,y=10+Math.floor(i/2)*160;return `<g transform="translate(${x} ${y})"><text x="0" y="25" font-size="24" fill="#173955">${['①','②','③','④'][i]}</text><path d="M48 20v100h195" stroke="#173955" stroke-width="2" fill="none"/><path d="${['M50 24L225 119','M50 84L225 20','M50 119L225 20','M50 42L225 42'][v]}" stroke="#cc6247" stroke-width="3" fill="none"/><text x="78" y="148" font-size="15">물체의 무게</text><text x="41" y="22" transform="rotate(-90 41 22)" text-anchor="end" font-size="14">늘어난 길이</text></g>`}).join(''),'네 가지 그래프 보기: 감소, 원점을 지나지 않는 증가, 원점을 지나는 증가, 일정');}
export function toolIcon(kind){
 let b='';
 if(kind==='pen')b='<path d="M61 44h144l47 42-47 41H61z" fill="#d7e5e8"/><path d="M65 85h161M226 73l33 12-33 12" stroke="#1d526d" fill="none" stroke-width="9"/><path d="'+coil(157,73,55,25,8)+'" stroke="#d3684c" fill="none" stroke-width="3" transform="rotate(90 184 86)"/>';
 if(kind==='stapler')b='<path d="M44 111h217l-18 18H45z" fill="#376276"/><path d="M56 102L58 53Q165 25 259 59L244 88L74 87z" fill="#217978"/><path d="M77 98L90 75 101 100 113 76 123 101" fill="none" stroke="#da694c" stroke-width="4"/>';
 if(kind==='trampoline')b='<ellipse cx="150" cy="76" rx="118" ry="39" fill="#25656c"/><ellipse cx="150" cy="76" rx="92" ry="28" fill="#173b52"/><path d="M47 89v57m205-57v57M85 112v39m134-39v39" stroke="#5b7484" stroke-width="7"/>';
 if(kind==='expander')b='<rect x="25" y="41" width="41" height="91" rx="16" fill="none" stroke="#214962" stroke-width="12"/><rect x="234" y="41" width="41" height="91" rx="16" fill="none" stroke="#214962" stroke-width="12"/>'+[60,85,110].map(y=>'<path d="M67 '+y+' '+Array.from({length:24},(_,i)=>'L'+(67+i*7)+' '+(y+(i%2?7:-7))).join(' ')+'L234 '+y+'" fill="none" stroke="#b28656" stroke-width="4"/>').join('');
 if(kind==='pogo')b='<path d="M147 24v130M109 27h76M98 113h104" stroke="#1b4c68" stroke-width="10" stroke-linecap="round"/><path d="'+coil(128,62,38,65,9)+'" stroke="#da694c" stroke-width="4" fill="none"/>';
 if(kind==='trap')b='<path d="M62 45h177v106H62z" fill="#cfad77"/><path d="M88 120V63h120v57Z" stroke="#466677" stroke-width="7" fill="none"/><path d="'+coil(125,91,44,18,8)+'" fill="none" stroke="#d76a4a" stroke-width="4"/>';
 if(kind==='leaf')b='<path d="M31 78Q150 161 269 78M47 75Q150 140 253 75M66 72Q150 120 234 72M93 71Q150 104 207 71" stroke="#446c80" stroke-width="8" fill="none"/><rect x="143" y="87" width="17" height="51" rx="4" fill="#db714e"/>';
 if(kind==='watch'){b='<circle cx="150" cy="85" r="67" fill="#d8e2e5" stroke="#31546a" stroke-width="5"/>';let d='';for(let i=0;i<400;i++){let t=i/399,r=t*53,a=t*12*Math.PI;d+=(i?'L':'M')+(150+r*Math.cos(a))+' '+(85+r*Math.sin(a));}b+='<path d="'+d+'" fill="none" stroke="#b07f42" stroke-width="3"/>';}
 if(kind==='kitchen')b='<ellipse cx="150" cy="34" rx="80" ry="17" fill="#bacfd6"/><path d="M97 50h108l30 101H69z" fill="#217a79"/><circle cx="150" cy="104" r="35" fill="#fff"/><path d="M150 104l16-22" stroke="#d56649" stroke-width="5"/>';
 if(kind==='body')b='<path d="M49 44Q150 11 251 44L235 147H64Z" fill="#d9e6e7" stroke="#517285" stroke-width="3"/><rect x="118" y="42" width="63" height="30" rx="5" fill="#17394f"/><text x="150" y="64" text-anchor="middle" fill="#fff" font-size="17">kg</text>';
