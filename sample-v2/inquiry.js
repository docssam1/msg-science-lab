...
import {inquiryObjects,inquiryScale,inquiryStateKey,initialOrder,validOrder,formatForce,objectFigure} from './everyday-objects.js';
...
export async function mountInquiry(host,ctx){
 ...
 const state=old&&Array.isArray(old.attempts)&&validOrder(old.order)&&old.attempts.every(r=>inquiryObjects.some(o=>o.id===r.id))?old:{phase:'predict',order:initialOrder(),uncertain:false,reason:'',reflection:'',attempts:[],retry:false};
 ...
 function prediction(){
  ...
  function drawOrder(){orderHost.innerHTML=state.order.map((id,i)=>`<div class="order-card" data-order-id="${id}"><span class="order-position">${i+1}</span><div class="order-handle" role="button" tabindex="0" aria-label="${obj(id).name} 끌어 옮기기">${objectFigure(obj(id))}<b>${obj(id).name}</b></div><div><button data-earlier="${id}" ${i===0?'disabled':''} aria-label="${obj(id).name} 더 가벼운 쪽으로">←</button><button data-later="${id}" ${i===state.order.length-1?'disabled':''} aria-label="${obj(id).name} 더 무거운 쪽으로">→</button></div></div>`).join('');
 ...
 async function measurement(){
  ...
  panel.querySelector('[data-inquiry-remove]').onclick=()=>{lab.setForce(0);panel.querySelector('[data-inquiry-object]').disabled=true;selected=null;adjustedLoaded=false;panel.querySelector('[data-inquiry-reading]').value='';panel.querySelectorAll('[data-object]').forEach(x=>x.setAttribute('aria-pressed','false'));feedback('물체를 뺐습니다.');};
 ...
 function summary(){
  ...
  host.innerHTML=`<div class="inquiry-stage inquiry-summary"><div class="eyebrow">마지막 정리 · 이제 원인 공개</div><h3>예상과 측정 결과를 비교해요.</h3><p><b>처음 예상</b> ${esc(forecast())}</p><div class="inquiry-pair everyday-results">${inquiryObjects.map(o=>`<div>${objectFigure(o)}<b>${o.name} · ${formatForce(latest.get(o.id).value)} N</b></div>`).join('')}</div><h3>가벼운 순서 → 무거운 순서</h3><p class="inquiry-ranking">${measured.map(o=>`${o.name} ${formatForce(latest.get(o.id).value)} N`).join(' → ')}</p><p>예상이 달랐다는 것만으로 실험 방법이 잘못된 것은 아닙니다. 과정이 올바른 측정값으로 생각을 고치면 됩니다.</p><h3>실험 과정을 돌아봐요.</h3>${failureReasons.size?[...failureReasons].map(k=>`<p>${explanations[k]}</p>`).join(''):'<p>제출한 기록에서 점검 대상인 실험 방법 오류는 발견되지 않았습니다.</p>'}<p><b>기억할 점</b> 모든 물체의 측정값에 같은 오차가 더해지면 순서는 그대로일 수도 있어요. 줄 세운 순서만 맞았다고 측정값까지 정확한 것은 아닙니다.</p><label class="inquiry-label">그래서 저울은 왜 필요할까요?<textarea data-conclusion rows="3" maxlength="500">${esc(state.conclusion||'')}</textarea></label><button data-save-conclusion>나의 결론 저장</button><p class="inquiry-note">물체와 수치는 추가 가상 탐구의 설정이며 제품의 실측값이 아닙니다. 귤 두 개는 한 묶음으로 잽니다. 같은 투명 받침의 무게를 제외해 물건만 비교하는 가상 설정입니다. 실제 수업에서는 받침의 무게 조건도 같게 합니다.</p></div>`;
