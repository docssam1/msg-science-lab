...
`npm start` 후 `/sample-v2/index.html`을 열어 `스스로 공부하기`, `가르치기`, `살아있는 책`을 선택합니다. 세 화면은 같은 P0–P23 학생 지면을 사용하며, 인쇄 메뉴는 학생용 A4 24면 또는 학생 지면과 교사 노트를 짝지은 강사용 A4 47면을 만듭니다. 교사 공개 기록은 학생 기록과 분리됩니다.
...
- P3–P5·P7·P19의 추가 사진은 `photos/CREDITS.md`에 촬영자, 원본, 라이선스를 적었습니다. 사진 속 물건의 실제 무게와 앱의 가상 측정값은 다릅니다. P19 완력기는 구조 개념 도해입니다.
...
실제 검수 결과와 남은 판단은 `docs/lesson-framework/DESIGN-QA.md`에 기록합니다. 본문 p17의 반비례 표현, 평가 9–11번 해설, 정확한 대상 학년과 도구 사용 지도 조건은 검토 보류입니다. QR은 현재 공개 사이트 URL을 담고 있으므로 이 브랜치가 공개되기 전에는 해당 공개 사이트가 새 P5 동작을 제공한다는 뜻이 아닙니다. 검수 완료 전 `main` 병합·배포 금지입니다.
