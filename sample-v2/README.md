...
- 강의용은 현장 수업을 진행하는 교안입니다. `2팀 배틀`을 켜면 같은 가상 저울 두 개가 나란히 열리고, A·B팀이 다섯 물건을 각자 줄 세우고 측정합니다. `새 대결 · 다른 물건`을 누를 때마다 기존 생활 물건 → 볼펜·스테이플러를 넣은 교실·생활 물건 → 다른 생활 물건 조합의 **3세트가 순환**하며 두 팀에 같은 세트가 주어집니다. 팀별 예상·기록은 새 대결마다 초기화됩니다. 교사는 가상 교구 설정값으로 정답 ○/×를 공개하거나, 교실 실물 결과를 보고 직접 ○/×를 표시합니다. 실물의 무게는 화면이 자동 판정하지 않습니다.
